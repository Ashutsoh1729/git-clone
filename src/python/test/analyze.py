...
def build_tree(
    ...
    """
    # as it require entries to build the tree from

    if not entries:
        raise ValueError("No entries to build tree from")

    # Group entries into files (direct children) and subdirs
    # why it is a dictionary rather than a list
    subdirs: Dict[str, List[Tuple[str, str, str]]] = {}
    files: List[Tuple[str, str, str]] = []  # (name, mode, sha)

    # For filtering out the strings entries
    # first item is full_path , second one is mode and third one is sha ( hash, which is it's address )
